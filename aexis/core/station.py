...
class Station(EventProcessor):
    ...
    async def _handle_passenger_event(self, data: Dict):
        ...
        try:
            ...
            logger.debug(f"Station {self.station_id} passenger event error: {e}", exc_info=True)
    ...
    async def _handle_cargo_event(self, data: Dict):
        ...
        try:
            ...
            logger.debug(f"Station {self.station_id} cargo event error: {e}", exc_info=True)
    ...
    async def _handle_passenger_arrival(self, event_data: Dict):
        ...
        logger.info(f"Station {self.station_id}: Passenger {passenger_id} added to queue (queue size: {len(self.passenger_queue)})")
    ...
    async def _handle_cargo_request(self, event_data: Dict):
        ...
        logger.info(f"Station {self.station_id}: Cargo {request_id} added to queue (queue size: {len(self.cargo_queue)})")
    ...
    async def _handle_capacity_update(self, data: Dict):
        ...
        try:
            ...
            self.processing_rate = parameters.get('processing_rate', self.processing_rate)
            
            logger.info(f"Station {self.station_id}: Capacity updated - bays: {self.loading_bays}, rate: {self.processing_rate}")
            
        except Exception as e:
            logger.debug(f"Station {self.station_id} capacity update error: {e}", exc_info=True)
    
    def _update_congestion_level(self):
        """Calculate current congestion level"""
        # Base congestion from queue lengths
        passenger_congestion = min(1.0, len(self.passenger_queue) / 20.0)  # 20 passengers = full congestion
        cargo_congestion = min(1.0, len(self.cargo_queue) / 10.0)  # 10 cargo = full congestion
    ...
    def _update_wait_time_metrics(self):
        ...
        for passenger in self.passenger_queue:
            wait_time = (current_time - passenger['arrival_time']).total_seconds() / 60  # minutes
    ...
    async def _publish_congestion_alert(self):
        ...
        affected_routes = [f"{self.station_id}->{station}" for station in self.connected_stations]
        ...
        estimated_clear_time = datetime.utcnow() + timedelta(minutes=estimated_clear_minutes)
        ...
        logger.warning(f"Station {self.station_id}: Congestion alert - level {self.congestion_level:.2f}")
...
class PassengerGenerator:
    ...
    async def _create_passenger(self, origin: str):
        ...
        passenger_id = f"p_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_{random.randint(1000, 9999)}"
...
class CargoGenerator:
    ...
    async def _create_cargo_request(self, origin: str):
        ...
        if random.random() < 0.3:
            deadline = datetime.utcnow() + timedelta(hours=random.randint(2, 24))
        
        request_id = f"c_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_{random.randint(1000, 9999)}"
