...
class ErrorCode(Enum):
    ...
    GEMINI_RATE_LIMIT_EXCEEDED = 1202
    GEMINI_QUOTA_EXCEEDED = 1203
    GEMINI_MODEL_NOT_AVAILABLE = 1204
    GEMINI_RESPONSE_PARSING_FAILED = 1205
    GEMINI_TIMEOUT = 1206
    GEMINI_DAILY_LIMIT_REACHED = 1207
...
class AexisException(Exception):
    ...
    def __init__(self, error_code: ErrorCode, message: str, component: str = "unknown", 
...
class GeminiException(AexisException):
...
ERROR_MESSAGES = {
    ...
    ErrorCode.INVALID_CONFIGURATION: "Invalid configuration: {details}",
    
    ErrorCode.REDIS_CONNECTION_FAILED: "Failed to connect to Redis at {host}:{port}: {reason}",
    ErrorCode.REDIS_PUBLISH_FAILED: "Failed to publish message to channel {channel}: {reason}",
    ErrorCode.REDIS_SUBSCRIBE_FAILED: "Failed to subscribe to channel {channel}: {reason}",
    ErrorCode.REDIS_AUTHENTICATION_FAILED: "Redis authentication failed: {reason}",
    ErrorCode.REDIS_TIMEOUT: "Redis operation timed out after {timeout}s",
    ErrorCode.REDIS_CHANNEL_NOT_FOUND: "Redis channel {channel} not found",
    
    ErrorCode.GEMINI_API_KEY_MISSING: "Gemini API key not configured",
    ErrorCode.GEMINI_API_KEY_INVALID: "Gemini API key is invalid or expired",
    ErrorCode.GEMINI_RATE_LIMIT_EXCEEDED: "Gemini API rate limit exceeded. Retry after {retry_after}s",
    ErrorCode.GEMINI_QUOTA_EXCEEDED: "Gemini API quota exceeded. Quota resets at {reset_time}",
    ErrorCode.GEMINI_MODEL_NOT_AVAILABLE: "Gemini model {model} is not available",
    ErrorCode.GEMINI_RESPONSE_PARSING_FAILED: "Failed to parse Gemini API response: {reason}",
    ErrorCode.GEMINI_TIMEOUT: "Gemini API request timed out after {timeout}s",
    ErrorCode.GEMINI_DAILY_LIMIT_REACHED: "Pod {pod_id} reached daily API limit of {limit} calls",
    ...
    ErrorCode.EVENT_DATA_INVALID: "Event data is invalid: {reason}",
    ...
    ErrorCode.VALIDATION_INVALID_TYPE: "Invalid type for field {field_name}: expected {expected_type}, got {actual_type}",
    ErrorCode.VALIDATION_OUT_OF_RANGE: "Value {value} for field {field_name} is out of range [{min}, {max}]",
    ErrorCode.VALIDATION_INVALID_FORMAT: "Invalid format for field {field_name}: {reason}",
    ErrorCode.VALIDATION_DUPLICATE_ID: "Duplicate ID found: {id_type} {id}",
}


def create_error(error_code: ErrorCode, component: str = "unknown", 
                 context: Optional[Dict[str, Any]] = None, **kwargs) -> AexisException:
